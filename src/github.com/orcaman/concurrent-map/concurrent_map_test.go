...
func TestOneWriteAndMultiRead(t *testing.T) {
	wg := &sync.WaitGroup{}
	m := New()
	wg.Add(4)
	iterations := 1000000
	...
	go func() {
		...
		fmt.Println("routing 1 finished")
		wg.Done()
	}()
	go func() {
		for i := 0; i < iterations; i++ {
		...
		fmt.Println("routing 2 finished")
		wg.Done()
	}()
	go func() {
		for i := 0; i < iterations; i++ {
		...
		fmt.Println("routing 3 finished")
		wg.Done()
	}()
	go func() {
		for i := 0; i < iterations; i++ {
		...
		fmt.Println("routing 4 finished")
		wg.Done()
	}()
	wg.Wait()
